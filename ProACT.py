...
def prioritize(tree,n):
    ...
    leaves = list(); root_dist = dict(); root_dist_u = dict(); score = dict()
    for u in tree.traverse_preorder():
        ...
        if u.is_leaf():
            leaves.append(u); score[u] = (u.edge_length, root_dist[u]/root_dist_u[u])
    if n == 'All':
        n = len(leaves)
    else:
        n = int(n)
    ...
    return [l.label for l in sorted(leaves, key=lambda x: score[x])[:n]]
